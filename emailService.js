...
async function getTransporter() {
    ...
    // Check if custom SMTP is configured in .env
...
async function sendStage1InitialEmail(booking) {
    ...
    const fromAddress = process.env.EMAIL_FROM || '"Explore Galiyat Reservations" <reservations@exploregaliyat.com>';
...
async function sendStage1AdminNotification(booking) {
    const mailer = await getTransporter();
    const adminEmail = process.env.ADMIN_NOTIFICATION_EMAIL || process.env.SMTP_USER || 'admin@exploregaliyat.com';
    const fromAddress = process.env.EMAIL_FROM || '"Explore Galiyat System" <system@exploregaliyat.com>';
